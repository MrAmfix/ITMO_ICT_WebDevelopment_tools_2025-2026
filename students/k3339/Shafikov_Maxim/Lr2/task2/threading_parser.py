# Task 2 — parallel web page parsing using threading
# Uses requests + BeautifulSoup + ThreadPoolExecutor + psycopg2.
...
# Parse sync DSN from async URL: postgresql+asyncpg://user:pass@host:port/db -> postgresql://user:pass@host:port/db
...
def main():
    init_table()
    print(f"Parsing {len(URLS)} URLs with {NUM_WORKERS} threads...")

    start_time = time.perf_counter()

    results = []
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        future_to_url = {executor.submit(parse_and_save, url): url for url in URLS}
